...
@section('header')
    <h1>{!! $slots['header'] ?? '¡Gracias por tu Orden!' !!}</h1>
@endsection

@section('content')
    <!-- Main Message Slot -->
    <div class="mb-20">
        {!! $slots['main_message'] ?? '<p><strong>Hemos recibido tu orden exitosamente.</strong></p><p>Estamos esperando la confirmación de tu pago para comenzar a procesar tu pedido.</p>' !!}
    </div>
    ...
    <div class="info-box">
        ...
        <p><strong>Fecha:</strong> {{ $order->created_at?->format('d/m/Y H:i') ?? 'N/A' }}</p>
    ...
    <div class="info-box">
        ...
        <p><strong>Costo de Envío:</strong> {{ $shipment['price_formatted'] }}@if(!empty($shipment['is_external'])) (pago al recibir, no incluido en el total)@endif</p>
        @endif

        @if(!empty($shipment['is_external']))
        <p><strong>Aviso de envío externo:</strong> El costo del envío no está incluido en el total de tu pedido. Deberás pagarlo directamente al mensajero al recibir el pedido.</p>
        @if(!empty($shipment['external_payment_instructions']))
        <p><strong>Instrucciones de pago del envío:</strong> {{ $shipment['external_payment_instructions'] }}</p>
        @endif
        @endif
...
@section('footer')
    {!! $slots['footer'] ?? '<p>Gracias por confiar en nosotros.<br>Te notificaremos cuando recibamos tu pago.<br>Este es un correo automático, por favor no responder.</p>' !!}
@endsection
